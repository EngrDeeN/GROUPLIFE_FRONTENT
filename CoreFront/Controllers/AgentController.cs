...
namespace CoreFront.Controllers
{
    public class AgentController : Controller
    {
        ...
        private readonly string Create_Agent = "http://"+Result_API+"/api/Agent/PostAgent";
        private readonly string Update_Agent = "http://"+Result_API+"/api/Agent/PutAgent";
      
        IConfiguration configuration;
        static string Result_API = "", IP_Address = "", Port_No = "";
        public AgentController(IConfiguration _configuration)
        {
            ...
            using (var client1 = new HttpClient())
            {
                ...
                if (agentRegister.FSAG_AGENT_CODE <= 0)
                {
                    try
                    {
                        ...
                        using (var response = await client1.PostAsync(Create_Agent, SendRequest))
                        {
                            string apiResponse = await response.Content.ReadAsStringAsync();
                            TempData["Agent_Registration"] = " " + apiResponse.Replace('"', ' ').Trim();
                            ...
                            foreach (JObject AgentParameter in dict2.Children<JObject>())
                            {
                                if (AgentParameter != null)
                                {
                                    ...
                                    TempData["Agent_Registration"] = "Agent Successfully Created.";
                                }
                            }
                        }
                    ...
                    {
                        TempData["Agent_Registration"] = ed.ToString();
                    }
                }
                else
                {
